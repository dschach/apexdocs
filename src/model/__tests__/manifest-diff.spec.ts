...
it('detects when new types are added', () => {
  ...
  const newManifest = new Manifest([commonType, addedType]);

  const typesDiff = ManifestDiff.build(originalManifest, newManifest)?.types;

  // expect(added.length).toBe(1);
  // expect(added[0].name).toBe('NewEnum');
});
//
